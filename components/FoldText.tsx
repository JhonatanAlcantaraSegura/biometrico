  ...
  value.split(/(\n)/).map((part, index) => {
    ...
    return (
      <span className="fold-text-whitespace" key={`${key}-space-${index}`}>
        {/* Espacio normal, no `\u00A0`: con el no separable el salto de linea caia antes del
            espacio y la linea siguiente empezaba con un hueco. Con `pre-wrap` el espacio normal
            se queda colgando al final de la linea anterior. */}
        {part}
      </span>
    );
  });
